  ...
  if (props.purchase?.id) {
    ...
    watch(form.purchases_items, (currentValue) => {
      ...
      for (const purchaseRecord of currentValue) {
        ...
        purchaseRecord.total = round(purchaseRecord.unit_price * purchaseRecord.quantity);
...
const searchForCustomers = _.debounce((customerSearchTerm) => {
  if (customerSearchTerm.trim()) {
    axios.get(route('customers.index', {'name': customerSearchTerm}))
      .then(response => {
        if (response.data.customers.length) {
          customers.value = mergeArraysOfObjects(customers.value, response.data.customers)
        }
      });
  }
...
const searchForProducts = _.debounce((productSearchTerm) => {
  if (productSearchTerm.trim()) {
    axios.get(route('products.index', {'name': productSearchTerm}))
      .then(response => {
        if (response.data.products.length) {
          products.value = mergeArraysOfObjects(products.value, response.data.products)
        }
      });
  }
}, SEARCH_DEBOUNCE_TIME_OUT);
...
<template class="overflow-auto">
  <SectionMain>
    ...
    <CardBox form @submit.prevent="submit">
      <p v-if="isShow && customers[0]" class=" pb-3 pt-3 mt-3 mb-3">Customer: {{ customers[0]['name'] }}</p>
      <div v-if="ADMIN_ROLE===currentUserRole">
        ...
        >
          <SearchDropdown
            ...
            :placeholder="customers[0]? customers[0]['name']:'Search for a customer and select it'"
            id="customer-name"
            :options="customers"
      ...
      <table>
        ...
        <tr v-for="(item,index) in form.purchases_items" :key="index">
          <td data-label="Product">
            <p v-if="isShow">{{ products.filter(product => product.id === item.product_id)[0]['name'] }}</p>
            <SearchDropdown
              v-if="!isShow"
              :placeholder="item.product_id? products.filter(product=>product.id===item.product_id)[0]['name']:'Search for the product then select'"
              :options="products"
      ...
      <div v-if="form.purchases_items.length" class="mt-6 border shadow-lg py-10 px-4 font-bold text-center">
        Total: {{ form.total }}
      </div>
      <div v-if="!form.purchases_items.length" class="mt-6 border shadow-lg py-10 px-4 font-bold text-center">
      ...
      <template #footer>
        <BaseButtons>
          <BaseButton v-if="!isShow" @click="submit" color="info" label="Save"/>
          <BaseButton type="button" :href="route('purchases.index')" color="info" outline label="Back"/>
        </BaseButtons>
      </template>
    </CardBox>
  </SectionMain>
